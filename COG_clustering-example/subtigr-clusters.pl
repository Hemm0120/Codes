#!/usr/bin/env perl
## Set Variables
my $distancestf_subrole_dir = "Distances";
my $clusterstf_subrole_dir  = "Clusters";
my $r_scripts     = "Rscripts";
for my $dir2make ( "$distancestf_subrole_dir", "$clusterstf_subrole_dir", "$r_scripts" ) {
    unless( -d "$dir2make" ) {
        mkdir("$dir2make");
    }
}
...
xmat = as.matrix(dx)
write.table(xmat,
        "$distancestf_subroletf_subrole_dir/Bacilloma-$dist.matrix",
        row.names=T,col.names=T,quote=F)
#### the output file contains the distance matrix as a table
xmat[upper.tri(xmat)] <- NA
x = melt(xmat, na.rm=T)
write.table(x, "$distancestf_subrole_dir/Bacilloma-$dist.tbl",
...
cogmat <- as.matrix(read.table("$distancestf_subrole_dir/Bacilloma-$dist.matrix"))
...
write.tree(phy, file="$clusterstf_subrole_dir/$dist.$method.newick")
