#!/usr/bin/env perl
## Set Variables
my $distances_dir = "Distances_subrole";
my $clusters_dir  = "Clusters_subrole";
my $r_scripts     = "Rscripts_subrole";
...
xmat = as.matrix(dx)
write.table(xmat,
        "$distances_dir/TIGR_subrole-$dist.matrix",
        row.names=T,col.names=T,quote=F)
#### the output file contains the distance matrix as a table
xmat[upper.tri(xmat)] <- NA
x = melt(xmat, na.rm=T)
write.table(x, "$distancestf_subrole_dir/TIGR_subrole-$dist.tbl",
...
cogmat <- as.matrix(read.table("$distances_dir/TIGR_subrole-$dist.matrix"))
