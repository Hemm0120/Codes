##### this R script will produce manhattan distances from the
...
############## import the TIGR data
    ...
    dx <- dist(data,method="manhattan")
    ## dx <- dist(cogtbl,method="manhattan") ## Create Distance matrix of the data
...
xmat = as.matrix(dx)
write.table(xmat,
        "Distances_subrole/TIGR_subrole-manhattan.matrix",
        row.names=T,col.names=T,quote=F)
#### the output file contains the distance matrix as a table
xmat[upper.tri(xmat)] <- NA
x = melt(xmat, na.rm=T)
write.table(x, "/TIGR_subrole-manhattan.tbl",
